...
async function loadAppointments() {
  ...
  if (error) {
    ...
    appointmentsList.innerHTML = "<p>Could not load appointments.</p>";
    return;
  }
  ...
  appointments.forEach(function (appointment, index) {
    ...
    card.innerHTML = `
      ...
      <div id="edit-${index}" class="appointment-edit" style="display:none;">
        ...
        <input type="text" id="polish-${index}" value="${appointment.polish || ""}">
...
window.saveAppointment = async function (index) {
  ...
  const { error } = await supabaseClient
    ...
    .update({
      ...
      polish: document.getElementById(`polish-${index}`).value,
